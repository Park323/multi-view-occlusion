...
def load_max_boundary(boundary_path):
    # Read ROI
    ...
    return xtl, ytl, xbr, ybr, boundaries
...
def main(args):
    ...
    for input, boundary_path in zip(args.input, args.boundary_path):
        video, fps, width_crop, height_crop = capture_video(input, 100)
        video_lst.append(video)
        
        *max_boundary, boundaries = load_max_boundary(boundary_path)
        max_boundaries.append(max_boundary)
        boundaries_lst.append(boundaries)
    height = 1920
    width = 1080
    height_crop, width_crop = map(int, (height_crop, width_crop))

    # 2. Pixel-system
    world2pixel = width / args.sensor_size[0]
    f_pixel = args.focal_length * world2pixel
    
    xtl, ytl, xbr, ybr = max_boundaries[args.target_idx]
    ...
    for frame_idx, xyzt in enumerate(movement_generator):
        ...
        for i, (video, cam_pos, cam_ang) in enumerate(zip(video_lst, args.camera_principal_point, args.camera_rot)):
            ...
            y_img += width//2
            # Crop lip boundary
            xmin, ymin, xmax, ymax = boundaries_lst[i][frame_idx]
            roi_scale_factor = height_crop / (xmax - xmin)
            x_img = int((x_img - xmin) * roi_scale_factor)
            y_img = int((y_img - ymin) * roi_scale_factor)
            # Overlap
